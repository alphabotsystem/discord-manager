...
async def update_static_messages():
	...
	try:
		...
		if howtoMessage is not None: await howtoMessage.edit(content=None, embed=Embed(title="Best-in-class order execution client. Trade cryptocurrencies via Ichibot right in Discord.", description="[Sign up for a free account on our website](https://www.alpha.bot/signup). If you already signed up, [sign in](https://www.alpha.bot/login), connect your account with your Discord profile, and add an API key. All Ichibot commands are prefixed with `x`. Learn more about Ichibot on their [GitLab page](https://gitlab.com/Ichimikichiki/ichibot-client-app/-/wikis/home).", color=0x673AB7), suppress=False)
